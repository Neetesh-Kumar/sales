...
$invoice_number="";
$lead_number="";
$query = "select * from invoice  where invoice_id = $_GET[eid]";
...
<?php
	// $query = "select * from sales_order order by order_number desc limit 1";
	// $query_run = mysqli_query($connection,$query);
  // $row = mysqli_fetch_array($query_run);
	// $last_nmbr = $row['order_number'];
	// if($last_nmbr == "")
	// {
	// 	$order_number = "Order-1";
	// }
	// else{
	// 	$order_number = substr($last_nmbr,3);
  ...
  // $connection = mysqli_connect("localhost","root","");
	// $db = mysqli_select_db($connection,"demo_crm");
		// include('conn.php');
	$last_nmbr = '';
	$query = "select * from sales_order order by order_number desc limit 1";
    ...
    if(mysqli_num_rows($stmt) > 0) {
        if ($row = mysqli_fetch_assoc($stmt)) {
            ...
            $last_nmbr = "ORD-" . sprintf('%04s', $last_nmbr);//concatenating incremented value
            $order_number = $last_nmbr;
        }
    } 
    else {
        $last_nmbr = "ORD-0001";
  ...
  <body>
              ...
              <div class="col-md-6 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <p class="card-description"> Add Order </p>
    <form action="create_order.php" method="post" enctype="multipart/form-data">
      ...
      <label for="text">Sales Person</label>
     	<input type="text" name="sales_person" class="form-control"  value="<?php echo $_SESSION['sales_name'];?>" readonly>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
          ...
          <select id="color" class="form-control currency-list" name="invoice_status" readonly>
          <option class="dark" value="0">Hold</option>
    ...
    <input type="Date" name="sale_startdate" class="form-control" required>

    </div>
    <div class="form-group">
    <label for="text">Delivery Date</label>
    <input type="Date" name="sale_enddate" class="form-control" required>

    </div>	
    <div class="form-group">
      <label class="form-label" for="customFile">Add File Attachment</label>
    <input type="file" name="pdf_order" class="form-control" id="customFile" />
    </div>
    <div class="form-group">
      <label class="form-label" for="customFile">Discription</label>
      <textarea class="form-control" id="discription" name="discription_order" id="exampleFormControlTextarea1" value="<?php echo $discription_order; ?>" rows="5"></textarea>
    </div>
    </div>
      <div class="modal-footer">
   
      <button type="submit" name="login" class="btn btn-primary mr-2">Create Order</button>
		</form>
